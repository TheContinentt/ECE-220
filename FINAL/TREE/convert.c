...
void convertRecursive(node * current, node ** lastNode)
{
	...
	{
		convertRecursive(current->left, &current);
		current->left = NULL;
		convertRecursive(current->right, &current);
		
		
	/*node * prev = (*lastNode)->left;
	if ((*lastNode)->value > current->value)
	{
		current->right = *lastNode;
		current->left = (*lastNode)->left;
		(*lastNode)->left = current;
		prev->right = current;
	}
	else
	{
		convertRecursive(current, (*lastNode)->right);
	}*/
    // YOUR CODE HERE
}
