...
/*The following functions would utilize the concept of binary trees to find the least area of several composition of rectangles which is handy
in circuit design. The very several functions would tell what role is the given pointer and judged from that would swap the corresponding values.
 Then the last functions would traverse the postfix expressions and try to expend them and put it into recursion in a left-screwed way to find the least area.
*/
int is_leaf_node(node_t* ptr) {
  ...
  if(ptr->left==NULL && ptr->right==NULL&&ptr->parent!=NULL){
    return 1;//if they have no sons, they are single leaf
  }
  return 0;
}
...
int is_internal_node(node_t* ptr) {
  ...
  if(ptr==NULL){
    ...
    if(is_leaf_node(ptr)==1){//if it is not a leaf, then must be internal tree
      return 0;}
    return 1;
}
...
int is_in_subtree(node_t* a, node_t* b) {
  ...
  node_t *test;
  test=b;
  while(test!=NULL){//traverse to the top root as to find whether the b is at the same subtree as in a
    if(test==a){return 1;}
    test=test->parent;
  }
    return 0;
...
void rotate(node_t* ptr) {
  ...
  int temp;
  if(ptr!=NULL && (is_leaf_node(ptr)==1)){//swap the height and weight
  temp=ptr->module->w;
...
void recut(node_t* ptr) {
  ...
  // TODO:
  if(ptr->cutline==V){//swap the value of V and H
		ptr->cutline=H;
	}
...
void swap_module(node_t* a, node_t* b) {
  ...
  // TODO:
  module_t *ptr;//simple swap function that swap two pointer's value
  ptr=a->module;
  a->module=b->module;
  b->module=ptr;
}
...
void swap_topology(node_t* a, node_t* b) {
  ...
  // TODO:
      node_t* temp;//initialize a pointer to swap parents value
      if(a==a->parent->left){//a is at left
        if(b==b->parent->left){//b is at left
          temp = a->parent->left;
    ...
    }//when the swap subspring done, swap their mothers at last
      temp=a->parent;
      a->parent=b->parent;
      b->parent=temp;
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  // TODO:
  postfix_traversal(ptr->left,nth,expression);//traversing the left tree
  postfix_traversal(ptr->right,nth,expression);//traversing the right tree
  expression[*nth].module=ptr->module;//assignate the data from current ptr to the array
  expression[*nth].cutline=ptr->cutline;
  *nth=*nth+1;//update address
}
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  if(n==num_modules-1){
    par->module = modules+n;// the last recursion found the last left leaf and assignate last module value to it
    par->cutline = UNDEFINED_CUTLINE;//the last leaf has no cutline value
    return NULL;
  }
  if(n==0){//at the beginning, initialize the binary tree
    par=(node_t*)malloc(sizeof(node_t));
    par->parent=NULL;
    par->cutline=V;//from the website, the initialization should be set to be V
    par->module=NULL;
    par->left=NULL;
    par->right=NULL;
  }
      ...
      par->right->parent=par->left->parent=par;
      par->right->right=par->right->left=par->left->right=par->left->left=NULL;
      par->left->module=NULL; //according to website, left leaf has no module value
      par->right->module=modules+n;//the right module increases subsequently
      par->left->cutline=V;//if module is null, initialize the cutline to V
      par->right->cutline=UNDEFINED_CUTLINE;//if module is not null, initialize the cutline as undefined
  if(init_slicing_tree(par->left, n+1)==NULL){//call the next left leaf with updated n value
  return par;
}
  return par;
}
